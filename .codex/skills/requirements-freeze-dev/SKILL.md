...
- 日志验证策略可执行（日志位置、关键 event/字段、触发动作、预期结果）。
...
- 文档必须包含：背景、目标、范围、契约、方案、测试计划、日志验证计划、风险、验收标准、变更控制。
...
- 功能实现时按需打印日志，至少覆盖改动对应的关键路径（开始/分支/完成或失败）。
- 日志必须可用于验证改动是否生效：明确 event 名、关键字段、预期值，并执行触发动作验证日志实际出现。
- 若新增日志仅用于本次调试，验收后删除或降级，避免长期噪声。
...
- 附上日志验证证据：触发命令/步骤、关键日志字段和值、与预期的一致性结论。
