...
- Thought uses chat tool-calling with tools: `schedule|internet_search|ask_user|done` + `todo` group（展开为 `todo_add|todo_list|todo_view|todo_get|todo_update|todo_delete|todo_done|todo_search`）+ `history` group（展开为 `history_list|history_search`）.
