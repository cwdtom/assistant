...
def _execute_typed_thoughts_system_action(
    *,
    ...
    if action == "get" and isinstance(arguments, ThoughtsIdArgs):
        ...
        if item is None:
            return _done_observation(agent=agent, action=action, raw_input=raw_input, result=f"未找到想法 #{arguments.id}")
        return _done_observation(
            agent=agent,
            action=action,
            raw_input=raw_input,
    ...
    if action == "delete" and isinstance(arguments, ThoughtsIdArgs):
        ...
        if not deleted:
            return _done_observation(agent=agent, action=action, raw_input=raw_input, result=f"未找到想法 #{arguments.id}")
        return _done_observation(agent=agent, action=action, raw_input=raw_input, result=f"想法 #{arguments.id} 已删除。")
